...
test.describe('Generate Build Flow & Validation', () => {
  ...
  test('Scenario 4: Rejects invalid budget boundary and prevents API call', async ({ page }) => {
    ...
    const generateBtn = page.getByRole('button', { name: /Generate Build/i });

    // Boundary Test: Invalid Number
  ...
  test('Scenario 1: Auto-generates a build and persists it to Saved Builds', async ({ page }) => {
    ...
    await page.getByRole('link', { name: 'Generate Build' }).click();
    await expect(page).toHaveURL(/.*generate/);

    await page.locator('#budget').fill('1500');
    await page.getByRole('button', { name: 'Generate Build' }).click();
    ...
    await expect(page.getByText('Generated Build ($1500)')).toBeVisible(); 
  });
